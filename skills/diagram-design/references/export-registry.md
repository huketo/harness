...
- **Duplicate `data-block-id` values**: emit every matching entry in document order; don't deduplicate or pick one. A duplicate ID is a correctness problem for the separate structural validation below, not for export to silently resolve.
- **`data-block-parent` pointing at an ID absent from the file, or a parent cycle**: emit the data exactly as authored, including the broken or cyclic reference. Same reasoning as duplicates — export mirrors the source, and structural validation is a separate, explicit step not implied by `--registry` itself.
...
- Validates ID uniqueness, parent resolution, or cycles. Export itself does not. In an installed skill, manually confirm that every ID is unique, every non-root parent identifies an emitted block, and following each parent chain terminates at a root without a cycle. `scripts/self_check.py` does not validate registry structure. From an optional upstream maintainer checkout, `python3 <repo-root>/scripts/verify-block-registry.py <html-file>` automates those checks.
