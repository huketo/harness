...
채널이 `afk`이거나 `herdr-hitl`이 종료 코드 `6`을 반환하면 `herdr-hitl` 스킬의 자율 결정·쿼럼·유예 정책을 적용합니다. 사람에게 질문·알림을 보내거나 재시도하지 않습니다. AFK는 새 권한을 주지 않으며 사전 승인은 유지합니다. 사람 전용 결정에 의존하는 작업만 유예하고, 독립적인 승인 작업은 계속하며 막힌 지점과 근거를 남깁니다.

`herdr-hitl afk`, `away`, `here`는 사람의 상태 선언입니다. 에이전트가 상태를 추측하거나 직접 전환하지 않습니다.

`herdr-cron`이나 벤치마크 러너처럼 스케줄러가 띄운 무인 실행에서는 `herdr-hitl`을 호출하지 않습니다. 사람만 결정할 수 있는 사항이 나오면 그 결정에 의존하는 작업만 멈추고 독립적인 승인 작업은 계속합니다. 작업 트리를 일관된 상태로 남긴 뒤, 막힌 지점과 근거를 실행 출력에 남깁니다.
